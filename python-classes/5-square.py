...
récupérer et modifier sa taille
avec des validations, de calculer son aire et d'afficher le
carré avec des symboles '#'.
...
Méthodes:
    ...
    my_print(): Affiche un carré formé de symboles '#' selon la taille
    spécifiée.
...
class Square:
    ...
    Méthodes:
        ...
        size: Propriété pour obtenir la taille du carré.
        size.setter: Propriété pour définir la taille du carré avec validation.
        my_print(): Affiche un carré formé de symboles '#' selon
        la taille spécifiée.
        area(): Calcule et retourne l'aire du carré.
    """
    ...
    def my_print(self):
        """
        Affiche un carré formé de symboles '#' selon la taille spécifiée.

        Si la taille du carré est 0, une ligne vide est imprimée.
        """
        if self.__size != 0:
            print(("#" * self.__size + '\n') * self.__size, end='')
        else:
